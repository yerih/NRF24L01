...
//Variables globales
uint8_t enviar[1], *recibido, *arreglo[5];
...
uint8_t ReadToNrf(uint8_t reg)
{
  /* Esta funcion lee un registro del nRF*/
  CLEARBIT(PORTD, 2);                //CSN low - nRF empieza a escuchar por comando
  delayMicroseconds(10);            
  WriteByteSPI(R_REGISTER + reg);   
  delayMicroseconds(10);            
  reg = WriteByteSPI(NOP);          //Envia comando NOP 
  delayMicroseconds(10);
  SETBIT(PORTB, 2);                 //CSN High - nRF 
  return reg;                       // Retorna el registro leido.
}

void WriteToNrf(uint8_t reg, uint8_t Package)
{
  /*Esta funcion escribe en el registro "reg" el valor "package" */
  
  CLEARBIT(PORTB, 2);  //CSN low - nRF empiea a escuchar por comando
  delayMicroseconds(10);
  WriteByteSPI(W_REGISTER + reg);
  delayMicroseconds(10);
  WriteByteSPI(Package);  //Envia el paquete que se escribe en el registro
  delayMicroseconds(10);
  SETBIT(PORTB, 2);       //CSN High - nRF vuelve a estado de reposo
}

uint8_t *WriteToNrf(uint8_t ReadWrite, uint8_t reg, uint8_t *val, uint8_t antVal)
{
  /* Esta funcion es capaz de leer y escribir en el nRF y deberia ser
...
if(int_count0 == 2)
{
  ...
  cli();
  CE_LOW;    
  *val = WriteToNrf(R,R_RX_PAYLOAD,*val,3);
  delay(100);
  for(int i=0;i<3;i++)
  {
  ...
  *recibido = 0;
  ...
  {   
    ...
    CE_LOW;
      ...
      CSN_LOW;
      WriteByteSPI(R_RX_PAYLOAD);
      *recibido = WriteByteSPI(0xfe);
      CSN_HIGH;
      
    //CE_LOW;
    //Serial.println("status:");
    //*recibido = GetReg(STATUS);
